...
const doubleLetters = (char) => char.repeat(2);

const doubleLettersOf = function (strings) {
  return strings.map((string) => [...string].map(doubleLetters).join(""));
};
...
const splitWordsOf = function (strings) {
  return strings.map((word) => word.split(" "));
};
...
const countVowelsOf = function (strings) {
  return strings.map((string) => [...string].filter(isVowel).length);
};
...
const withoutVowelsOf = function (strings) {
  return strings.map((string) => [...string].filter(isConsonant).join(""));
};

// cumulative sums of [[1, 2, 3], [4, 5, 6]] => [[1, 3, 6], [4, 9, 15]]
// Example: cumulative sum of [1, 2, 3] is [1, 1+2, 1+2+3]
const cumulativeSum = function ([firstElement, ...restOfNumbers]) {
  return restOfNumbers.reduce(
    function (sumArray, element) {
      return [...sumArray, sumArray.at(-1) + element];
    },
    [firstElement]
  );
};
...
const removeDuplicates = function (string) {
  return [...string].reduce(pushElements, []).join("").slice(0, 3);
};

const uniqueCharactersOf = (strings) => strings.map(removeDuplicates);
...
const capitalizedString = function (string) {
  return string
    .split(" ")
    .map((string) => string.at(0).toUpperCase() + string.slice(1))
    .join(" ");
};
...
const wordLengthsOf = (strings) => strings.map(wordLength);
